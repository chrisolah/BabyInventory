-- ============================================================================
-- Migration 042 - wishlist slot filters
-- ============================================================================
-- Adds included_slots and skip_slots to wishlist_shares so parents can
-- control which clothing slot types (bodysuits, pajamas, etc.) appear on
-- their share link.
--
-- included_slots text[] — null = all clothing slots shown
--   When set, only clothing_items with slot_id IN included_slots are returned.
--
-- skip_slots text[] — null = nothing skipped
--   Clothing slots the parent is well-stocked on; filtered from results and
--   listed in the share payload so the recipient page can show a notice.
-- ============================================================================

alter table beta.wishlist_shares
  add column if not exists included_slots text[],
  add column if not exists skip_slots     text[];
