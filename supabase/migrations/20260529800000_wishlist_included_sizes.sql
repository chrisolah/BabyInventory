-- ============================================================================
-- Migration 043 - wishlist included_sizes
-- ============================================================================
-- Adds included_sizes to wishlist_shares so parents can control which
-- clothing size bands appear on their share link.
--
-- included_sizes text[] - null = all sizes shown
--   When set, only clothing_items with size_label IN included_sizes are shown.
-- ============================================================================

alter table beta.wishlist_shares
  add column if not exists included_sizes text[];
...
begin
  ...
  from (
    ...
    where ci.household_id = v_share.household_id
      ...
      -- Size include filter (included_sizes: null = all)
      and (
        v_share.included_sizes is null
        or ci.size_label = any(v_share.included_sizes)
      )
      -- Size skip filter (skip_sizes: well-stocked sizes)
      and (
